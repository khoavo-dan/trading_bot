...
def get_ucb_action(q_values, c, t, n_ta):
    return np.argmax(q_values.numpy()[0]+c*np.sqrt(np.log(t+1)/n_ta))
...
def plot_trading_strategy(environment, q_network):
    state = environment.reset()
    series = environment.series[:,0]
    buy_indices = []
    sell_indices = []
    ...
    plt.plot(series)
    plt.plot(buy_indices, series[buy_indices], "x")
    plt.plot(sell_indices, series[sell_indices], "o")
    plt.show()
